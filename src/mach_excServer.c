...
mig_internal kern_return_t __MIG_check__Request__mach_exception_raise_state_t(
    ...
    msgh_size = In0P->Head.msgh_size;
    if ((In0P->Head.msgh_bits & MACH_MSGH_BITS_COMPLEX) ||
        (msgh_size < (mach_msg_size_t)(sizeof(__Request) - 912)) ||
        (msgh_size > (mach_msg_size_t)sizeof(__Request)))
        return MIG_BAD_ARGUMENTS;
#endif /* __MigTypeCheck */
...
#if __MigTypeCheck
    if (In0P->codeCnt > 2) return MIG_BAD_ARGUMENTS;
    if (((msgh_size - (mach_msg_size_t)(sizeof(__Request) - 912)) / 8 <
         In0P->codeCnt) ||
        (msgh_size <
         (mach_msg_size_t)(sizeof(__Request) - 912) + (8 * In0P->codeCnt)))
        return MIG_BAD_ARGUMENTS;
    ...
    *In1PP = In1P = (__Request *)((pointer_t)In0P + msgh_size_delta - 16);
    ...
    if (In0P->NDR.int_rep != NDR_record.int_rep)
        __NDR_convert__int_rep__Request__mach_exception_raise_state_t__old_stateCnt(
          ...
          */
#if __MigTypeCheck
    if (In1P->old_stateCnt > 224) return MIG_BAD_ARGUMENTS;
    if (((msgh_size - (mach_msg_size_t)(sizeof(__Request) - 912)) / 4 <
         In1P->old_stateCnt) ||
        (msgh_size !=
         (mach_msg_size_t)(sizeof(__Request) - 912) + (4 * In1P->old_stateCnt)))
        return MIG_BAD_ARGUMENTS;
#endif /* __MigTypeCheck */
...
#pragma pack()
    ...
    OutP->RetCode = catch_mach_exception_raise_state(
        In0P->Head.msgh_request_port, In0P->exception, In0P->code,
        In0P->codeCnt, &In1P->flavor, In1P->old_state, In1P->old_stateCnt,
        OutP->new_state, &OutP->new_stateCnt);
    ...
    OutP->Head.msgh_size =
        (mach_msg_size_t)(sizeof(Reply) - 896) + (((4 * OutP->new_stateCnt)));
...
mig_external mig_routine_t mach_exc_server_routine(mach_msg_header_t *InHeadP) {
    ...
    if ((msgh_id > 2) || (msgh_id < 0)) return 0;
